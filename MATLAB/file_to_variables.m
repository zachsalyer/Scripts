...
function [x, headers] = file_to_variables(fileName)
    emptyFile = 0;
    if ischar(fileName)
        'Reading the file ...'
    ...
    if numel(data) < 1
        emptyFile = 1;
        'Something is wrong with the log file... no data found'
    end
    if emptyFile == 0
        headers = data(1,:);
        for i=1:length(headers)
            ...
            key = strsplit(key{1},' ');
            key = key{1};
            headers(i) = cellstr(key);
            values = cell2mat(values(2:end,1));
            if strcmp(class(values),'double') == 0
                values = str2num(values(1:end,1));
            end
            map.(key) = values;
        end
    end
    % map.('ZAxisRoll') Do not include units when accessing data
    % map.('Temp40')
    x = map;
...
% Copyright (c) 2008, Ashish Sadanandan
% All rights reserved.
