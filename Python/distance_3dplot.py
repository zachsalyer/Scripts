...
for n in range(steps):
    ...
    slope[n+1] = np.rad2deg(np.arctan2(altitude_2[n+1] - altitude_2[n],distance_2[n+1] - distance_2[n]))
    print slope[n+1]
...
sm = plt.cm.ScalarMappable(cmap=green_red, norm=plt.Normalize(np.min(altitude_2), np.max(altitude_2)))
sm.set_array(altitude_2)
...
cb.set_label("Altitude (meters)")
#cb.ax.invert_yaxis()
labels = []
ax.set_xlim(np.min(x),np.max(x))
#ax.set_ylim(np.min(y),np.max(y))
ax.set_ylim(-105.098619,-105.013763)
ax.set_zlim(np.min(z),np.max(z))
x2, y2, _ = proj3d.proj_transform(x,y,z, ax.get_proj())

#print x2
ax.scatter(x,y,z,t,c=altitude_2[:-1],cmap=green_red, lw = 0, s = 5)
amount = 0
for i in range(0,np.size(z),800):
    name = "Altitude: " + np.str(z[i]) + " m\n" + "Distance: " + np.str(dist[i]) + " m\n" + "Slope: " + np.str(round(slope[i][0],2)) + " degrees"
    if amount%2 == 0:
        label = ax.annotate(name, xy=(x2[i], y2[i]),
                    xytext=(-200.0, -10.0), textcoords='offset points',
                    fontsize=13,                
        ...
        label = ax.annotate(name, xy=(x2[i], y2[i]),
                    xytext=(100.0, -10.0), textcoords='offset points',
                    fontsize=13,                
                    arrowprops=dict(arrowstyle="->",
                                    connectionstyle="arc3,rad=0")
                    )
...
plt.show()
ax.auto_scale_xyz([-0.035, 0], [38.3, 39], [3000, 4200])

plt.savefig('PP_map.png', transparent=True)
